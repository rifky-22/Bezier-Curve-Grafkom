...
void Demo::Update(double deltaTime) {
	...
	/*speed += (float)((deltaTime * 1.5f) / 500);
	if (updown >= 10 || updown < 0) {
		speedy *= -1;
	}
	updown += (float)((deltaTime * speedy) / 100); 
	*/
...
void Demo::Render() {
	...
	DrawColoredCube(8, 1, 1, 2, 4, 0, 1);
	DrawColoredCube(1, 3, 1, 2, 3, 0, 0);
	DrawColoredCube(4, 3, 4, 2, 1, 0, 0);
	DrawColoredCube(2, 2, 3, 0, 1, 0, 0);
	DrawColoredCube(7, 1, 1, 7, 1, 0, 0);
	DrawColoredCube(1, 3, 1, 9, 1, 1, 2);
	DrawColoredPlane();
...
void Demo::DrawColoredCube(float s1, float s2, float s3, float l1, float l2, float l3, float a)
{
	...
	model = glm::translate(model, glm::vec3(l1 - translateX, l2, l3 + translateZ));
...
int main(int argc, char** argv) {
	...
	app.Start("Transformation: Transform Cube", 1200, 720, false, false);
}